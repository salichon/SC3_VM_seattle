	...
	def Na(self, na):
		...
		if verboseFlag:
			if na.Key in self.att:
				print >>sys.stderr," Overridding key %s=%s with %s" % (na.Key,self.att[na.Key], na.Value)
			else:
		...
		if sa.match(self.code, self.location, None, self.start, self.end):
			if verboseFlag:
				if sa.Key in self.attLocation:
					print >>sys.stderr,"Overridding Location key %s=%s with %s" % (sa.Key, self.attLocation[sa.Key], sa.Value)
				else:
			...
			if sa.match(self.code, self.location, channel, self.start, self.end):
				if verboseFlag:
					if sa.Key in self.channels[channel].keys():
						print >>sys.stderr,"Overridding Channel key %s=%s with %s" % (sa.Key, self.channels[channel][sa.Key], sa.Value)
					else:
	...
	def __init__(self, line):
		...
		try:
			...
			if self.type != "S" and self.type != "L":
				raise Exception("Unknow callibration type %s" % self.type)
